...
float nvgTextInkBounds(NVGcontext* ctx, float x, float y, const char* string, const char* end, float* bounds)
{
	...
	// Unlike nvgTextBounds(), do NOT overwrite bounds[1]/bounds[3] with
	// fonsLineBounds(): keep fonsTextBounds()'s true ink y-extents so callers
	// can optically center a single glyph on its actual ink.
	width = fonsTextBounds(ctx->fs, x*scale, y*scale, string, end, bounds);
	if (bounds != NULL) {
		bounds[0] *= invscale;
		bounds[1] *= invscale;
